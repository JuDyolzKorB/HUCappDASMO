<?php
// components/patient_requisition_modal.php
$allPatients = get_data('patients');
$allItems = get_data('items');
?>
<div x-data="{
    isOpen: false,
    patients: <?php echo htmlspecialchars(json_encode($allPatients)); ?>,
    items: <?php echo htmlspecialchars(json_encode($allItems)); ?>,
    form: {
        patientId: '',
        diagnosis: '',
        notes: '',
    ...
    resetForm() {
        this.form = { patientId: '', diagnosis: '', notes: '', selectedItems: [] };
    },
    submit() {
        if (!this.form.patientId || this.form.selectedItems.length === 0) {
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl p-8 border border-slate-200 dark:border-slate-700">
            ...
            <div class="space-y-6">
                <!-- Select Patient -->
                <div>
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Patient</label>
                    <select x-model="form.patientId" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white">
                ...
                <!-- Add Items -->
                <div class="relative">
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Search Drug/Medicine</label>
                    <input type="text" x-model="searchItem" placeholder="Type medicine name..." class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white">
                    
                    <div x-show="filteredItems().length > 0" class="absolute z-10 w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl max-h-48 overflow-y-auto">
                        <template x-for="item in filteredItems()" :key="item.ItemID">
                            <div @click="addItem(item)" class="px-4 py-2 hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer text-slate-900 dark:text-white">
