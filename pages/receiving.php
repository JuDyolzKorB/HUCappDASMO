...
$posToReceive = array_filter($procurementOrders, function($po) {
...
function getPONumber($poid, $procurementOrders) {
    foreach ($procurementOrders as $p) {
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div class="space-y-1">
            ...
            <p class="text-slate-500 font-medium text-sm">
                <?php echo $tab === 'awaiting' ? 'Inspect and process incoming shipments from procurement orders.' : 'Logs of all recently received inventory items and batches.'; ?>
            </p>
        </div>
    ...
    <?php if ($tab === 'awaiting'): ?>
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700">
                <h3 class="text-lg font-semibold text-slate-800 dark:text-white">Procurement Orders - Approved for Receiving</h3>
            </div>
                        ...
                        <?php if (empty($posToReceive)): ?>
                            <tr>
                                <td colspan="5" class="px-6 py-8 text-center text-slate-500 dark:text-slate-400">
                                    No procurement orders are currently approved for receiving.
                                </td>
                            </tr>
                        <?php else: ?>
                    ...
                    <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
                        ...
                        <?php else: ?>
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-mono text-slate-500 dark:text-slate-400"><?php echo getPONumber($rec['POID'], $procurementOrders); ?></td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400"><?php 
                                    $userName = getUserFullName($rec['UserID'], $users);
                                    echo $userName; 
                                ?></td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <?php 
                                        $poNum = getPONumber($rec['POID'], $procurementOrders);
                                        $recJson = json_encode($rec);
                                    ?>
                                    <button 
                                        onclick='openReceivingDetailsModal(<?php echo $recJson; ?>, "<?php echo $poNum; ?>", "<?php echo $userName; ?>")' 
...
<?php 
// Include modals
include 'components/receiving_details_modal.php';
?>
