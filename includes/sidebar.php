...
$menuItems = [
    ...
    ['label' => 'Local Requests', 'id' => 'hc_requisitions', 'icon' => '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path></svg>', 'roles' => ['Administrator', 'Health Center Staff']],
    ...
    ['label' => 'HC Inventory', 'id' => 'hc_inventory', 'icon' => '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg>', 'roles' => ['Administrator', 'Health Center Staff']],
    ...
    ['label' => 'DPRI Import', 'id' => 'dpri_import', 'icon' => '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M9 19l3 3m0 0l3-3m-3 3V10"></path></svg>', 'roles' => ['Administrator', 'Head Pharmacist']],
        ...
        <nav class="space-y-1.5 px-3">
            <?php foreach ($menuItems as $item): ?>
                <?php if (in_array($userRole, $item['roles'])): ?>
                    <a href="index.php?page=<?php echo $item['id']; ?>" 
                       class="group flex items-center px-4 py-2.5 text-sm font-semibold rounded-xl transition-all duration-200 <?php echo $currentPage === $item['id'] ? 'sidebar-active' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white'; ?>">
                        <span class="<?php echo $currentPage === $item['id'] ? 'text-white' : 'text-slate-500 group-hover:text-teal-400'; ?> mr-3 flex-shrink-0 transition-colors">
                            <?php echo $item['icon']; ?>
                        </span>
                        <?php echo $item['label']; ?>
                    </a>
